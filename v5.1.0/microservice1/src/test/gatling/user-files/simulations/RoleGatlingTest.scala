...
class RoleGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Role entity")
        ...
        .repeat(2) {
            ...
            .get("/microservice1/api/roles")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new role")
            .post("/microservice1/api/roles")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "roleName":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_role_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created role")
                .get("/microservice1${new_role_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created role")
            .delete("/microservice1${new_role_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
